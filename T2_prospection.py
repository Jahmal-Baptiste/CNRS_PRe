...
if positions:
    with open("./T2/ThalamoCorticalModel_data_size_____/neuron_positions.pickle", "rb") as PyNN_file:
        while True:
            try:
                positions = pickle.load(PyNN_file, encoding="latin1")
                print(positions['exc'].shape)
                print(positions['inh'].shape)
            except EOFError:
                break